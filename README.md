...
- **动态技能路由**：Agent 在分析完新闻后，由 `PipelineRouter` 调用 LLM 自主决策是否执行 Reddit 检索和图像生成两个可选阶段，实现真正意义上的动态 Skill 选择
...
graph TD
    ...
    subgraph 路由决策层
    R{PipelineRouter\nDeepSeek-V3.2} -- run_reddit_search? --> C
    R -- run_image_generation? --> G
    end
    ...
    subgraph 4. 异步视觉化层
    G{是否生成图像?} -- 是 --> G1[DeepSeek-V3.2 视觉提示设计]
    G -- 否 --> J
    G1 -- 生成专业绘画提示词 --> H[异步调用 Qwen-Image-2512]
    H -- 轮询任务状态 --> I[下载 4K 梗图]
    end
...
│   └── pipeline_router.py            # 路由层：LLM 动态决策是否执行 Stage 2/4
...
├── main.py                            # 入口：四阶段 CLI 流水线 + 动态路由 + Rich UI
├── requirements.txt
...
传统 pipeline 的问题在于：无论输入什么新闻，都会盲目跑完所有阶段。NewsRoast-Agent 在 Stage 1 之后引入了一个独立的 **`PipelineRouter`**，由 LLM 基于分析结果做路由决策：

```
新闻分析完成
      ↓
PipelineRouter.decide(analysis_text)
      ↓ 调用 DeepSeek-V3.2
      ↓ 判断两个问题：
        1. 这条新闻在 Reddit 上有讨论价值吗？→ run_reddit_search
        2. 这条新闻有可视化梗图潜力吗？→ run_image_generation
      ↓ 返回 RoutingDecision + reasoning
      ↓
  Stage 2 / Stage 4 按需执行
```

**决策维度**：
- `run_reddit_search`：有效 SEARCH_QUERY + 属于国际/科技/商业话题 → True；纯本地新闻、无关键词 → False
- `run_image_generation`：分析含视觉元素或强视觉化槽点 → True；纯文字抽象议题 → False

**Fallback 保障**：路由器调用失败时自动降级为全执行，不影响主流程稳定性。

这使得 Agent 不再是 "sequential skill executor"，而是真正意义上的 **动态 skill selector**。
